...
public class AutonContainer {
  public static void CreateAutonChooser(LoggedDashboardChooser<Command> chooser) {
    ...
    // Set up auton routines for the selector. make sure that the auton names match the option names
    // to avoid confusion and typos
    ...
    // these 4 autons will be used in the event a team requests we have our robot do nothing for
    // auton, just in case
    ...
    // these two stay on one side and travel to neutral for collection
    chooser.addOption(
        "03: Depot Hub Score Neutral", new PathPlannerAuto("3-depot-hub-score-neutral"));
    chooser.addOption(
        "04: Human Hub Score Neutral", new PathPlannerAuto("4-human-hub-score-neutral"));
    // these two removed for now, as their paths take too long to complete
    /*
    chooser.addOption(
    ...
    */
    ...
    chooser.addOption(
        "07: Hub Neutral (Depot) Collect All Score",
        new PathPlannerAuto("7-hub-neutral-high-sweep-score"));
    chooser.addOption(
        "08: Hub Neutral (Human) Collect All Score",
        new PathPlannerAuto("8-hub-neutral-low-sweep-score"));
    ...
    // these two go to filling station and then score, refill in neutral, and then score again
    // before hanging
    chooser.addOption(
        "13: Lower Human Score Collect Score Hang",
        new PathPlannerAuto("13-lower-human-score-collect-score-hang"));
    chooser.addOption(
        "14: Upper Depot Score Collect Score Hang",
        new PathPlannerAuto("14-upper-depot-score-collect-score-hang"));
    // these two start at the hub, score, go to neutral, score again, then hang
    chooser.addOption(
        "15: Lower Hub Score Collect Score Hang",
        new PathPlannerAuto("15-lower-hub-score-collect-score-hang"));
    chooser.addOption(
        "16: Upper Hub Score Collect Score Hang",
        new PathPlannerAuto("16-upper-hub-score-collect-score-hang"));
  }
}
