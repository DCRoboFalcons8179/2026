...
public class HangerIOReal implements HangerIO {
  ...
  public void setPIDControl(double setpoint) {
    // set the setpoint for the PID controller, and calculate the output based on the current
    // position of the motor, and set the voltage to that output
  ...
  public boolean isMaxHeight() {
    // return whether or not the hanger is at its maximum height, which is when the encoder position
    // is at the maximum height, with some error margin to account for noise and overshooting
  ...
  public boolean isMinHeight() {
    // return whether or not the hanger is at its minimum height, which is when the encoder position
    // is at 0, with some error margin to account for noise and overshooting
    double omega = hangerMotor.getPosition().getValueAsDouble();
